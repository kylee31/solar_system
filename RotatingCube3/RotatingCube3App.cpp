...
void Display()
{
	...
	//glm::mat4 projection_matrix = glm::perspective(glm::radians(45.f), 1.f, 0.01f, 10000.f); 로 해보기
	//비율값을 잘 조절해야 하는 것을 알아야 한다
	//아래의 Viewpoint의 비율과 맞춰 주어야 함
	...
	//// 큐브 1. sun
	float scale_sun = 0.7f + 0.3f* abs(sin(g_elaped_time_s*2.f));
	glm::mat4 S = glm::scale(glm::vec3(scale_sun, scale_sun, scale_sun));
	glm::mat4 Sun = S;
	glUniformMatrix4fv(m_model_loc, 1, GL_FALSE, glm::value_ptr(Sun));
	DrawCube();


	//// 큐브 2. asteroids
	glm::mat4 R1_1 = glm::rotate(glm::radians(360.f*g_elaped_time_s*0.1f), glm::vec3(0.f, 1.f, 0.f));		// y축 중심으로 10초에 한바퀴(360도) 회전

	for (float i = 5.f; i <= 360.f; i += 5.f) {
		glm::mat4 S1 = glm::scale(glm::vec3(0.25f, 0.25f, 0.25f));
		glm::mat4 T1 = glm::translate(glm::vec3(5.f, 0.f, 0.f));
		glm::mat4 R1_2 = glm::rotate(glm::radians(0.f+i), glm::vec3(0.f, 1.f, 0.f));		// y축 중심으로 10초에 한바퀴(360도) 회전
		glm::mat4 Asteroids = R1_2 *R1_1 * T1 * S1;
		glUniformMatrix4fv(m_model_loc, 1, GL_FALSE, glm::value_ptr(Asteroids));
		DrawCube();
	}

	//// 큐브 3. earth
	glm::mat4 S2 = glm::scale(glm::vec3(0.45f, 0.45f, 0.45f));
	glm::mat4 R2_1 = glm::rotate(glm::radians(360.f*g_elaped_time_s*0.5f), glm::vec3(0.f, 1.f, 0.f));		// y축 중심으로 2초에 한바퀴(360도) 
	glm::mat4 T2 = glm::translate(glm::vec3(2.4f, 0.f, 0.f));
	glm::mat4 R2_2 = glm::rotate(glm::radians(360.f*g_elaped_time_s*0.05f), glm::vec3(0.f, 1.f, 0.f));		// y축 중심으로 20초에 한바퀴(360도) 
	glm::mat4 Earth = R2_2 * T2 * R2_1 * S2;
	glUniformMatrix4fv(m_model_loc, 1, GL_FALSE, glm::value_ptr(Earth));
	DrawCube();

	//// 큐브 4. moon
	glm::mat4 S3 = glm::scale(glm::vec3(0.2f, 0.2f, 0.2f));
	glm::mat4 T3 = glm::translate(glm::vec3(0.4f, 0.f, 0.f));
	glm::mat4 R = R2_2 * T2 * R2_1;
	glm::mat4 Moon = R * T3  * S3;
	glUniformMatrix4fv(m_model_loc, 1, GL_FALSE, glm::value_ptr(Moon));
	DrawCube();
...
void Reshape(int w, int h)
{
	...
	//glViewport(50, 50, 300, 300); 로 해보기 
	//이렇게 하면 결과값이 움직이지 않음 윈도우에 고정됨
